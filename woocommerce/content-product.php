...
<div class="product-inner">
	...
	<div class="product-image">
		...
		<a <?php echo porto_filter_output( $more_target ); ?> href="<?php echo esc_url( $more_link ); ?>" title="<?php echo get_the_title(); ?>">
			<?php
		...
		<?php
			...
			if (empty($custom_author)) {
				...
				if (!empty($terms)){
					...
					echo '<a href="'.$term_link.'">'.$terms[0]->name.'</a>';
					echo '</div>';
				}
			} else {
				echo '<div class="product-author">';

				$term_link = get_term_link($custom_author[0]);
				echo '<a href="'.$term_link.'">'.$custom_author[0]->name.'</a> ';
